...
# set model + acceptEdits, then prompt
send({"jsonrpc":"2.0","id":6,"method":"session/set_config_option","params":{"sessionId":sid,"configId":"model","value":"glm-5.3-flash:cloud"}})
time.sleep(1)
...
while True:
    ...
    if m.get("id") == 6: print("MODEL_SET:", "ok" if "error" not in m else str(m["error"])[:100], flush=True)
    if m.get("id") == 3:
